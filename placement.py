...
def overlap_repulsion_loss(cell_features, pin_features, edge_list):
    """Spatial-hash based overlap loss - O(N) average case instead of O(N²)."""
    ...
    # Use quadtree to find only overlapping pairs (O(N log N))
    positions = torch.stack([x, y], dim=1).detach().cpu().numpy()
    widths = w.detach().cpu().numpy()
    heights = h.detach().cpu().numpy()
    
    _, overlap_pairs = _quadtree_find_overlaps(positions, widths, heights, margin=0.1)
    
    if not overlap_pairs:
        return torch.tensor(0.0, device=cell_features.device, requires_grad=True)
    
    # Compute loss only for overlapping pairs
    pairs = list(overlap_pairs)
    i_idx = torch.tensor([p[0] for p in pairs], device=cell_features.device)
    j_idx = torch.tensor([p[1] for p in pairs], device=cell_features.device)
    ...
    ox = F.softplus(min_sep_x - dx, beta=10.0)
    oy = F.softplus(min_sep_y - dy, beta=10.0)
    
    loss = (ox * oy).pow(2).mean()
    return loss

def density_loss(cell_features, target_density=0.7, bins=16):
    """
    Smooth bin-based density overflow penalty.
    Prevents regional crowding at scale; O(N * bins).
    """
    device = cell_features.device
    N = cell_features.shape[0]
    
    # Limit bins for large designs to avoid memory issues
    if N > 10000:
        bins = min(bins, 8)
...
def train_placement(
    ...
    if N >= 1000:
        num_epochs_eff = 0
        constructive_iters = 1
        barycentric_passes = 2
        swap_top_k_edges = 200
        swap_max_count = 50
...
def legalize_placement(cell_features, margin=0.0, max_iters=None, step_frac=0.9, tol=1e-6):
    ...
    for _ in range(max_iters):
        positions = torch.stack([x, y], dim=1).detach().cpu().numpy()
        _, overlap_pairs = _quadtree_find_overlaps(positions, w.detach().cpu().numpy(), h.detach().cpu().numpy(), margin)
        
        if not overlap_pairs:
            break
        
        pairs = list(overlap_pairs)
        i_idx = torch.tensor([p[0] for p in pairs], dtype=torch.long, device=cf.device)
        j_idx = torch.tensor([p[1] for p in pairs], dtype=torch.long, device=cf.device)
        
        dxm = torch.abs(x[i_idx] - x[j_idx])
        dym = torch.abs(y[i_idx] - y[j_idx])
        minx = 0.5 * (w[i_idx] + w[j_idx]) + margin
        miny = 0.5 * (h[i_idx] + h[j_idx]) + margin
        
        penx = torch.clamp(minx - dxm, min=0)
        peny = torch.clamp(miny - dym, min=0)
        
        max_pen = max(penx.max().item(), peny.max().item())
        if max_pen < tol:
            break
        
        use_x = penx <= peny
        sx = torch.sign(x[i_idx] - x[j_idx])
        sy = torch.sign(y[i_idx] - y[j_idx])
        sx = torch.where(sx == 0, torch.ones_like(sx), sx)
        sy = torch.where(sy == 0, torch.ones_like(sy), sy)
        
        disp_x = 0.5 * penx * sx * use_x.float() * step_frac
        disp_y = 0.5 * peny * sy * (~use_x).float() * step_frac
        
        dx_acc = torch.zeros(N, device=cf.device)
        dy_acc = torch.zeros(N, device=cf.device)
        dx_acc.index_add_(0, i_idx, disp_x)
        dx_acc.index_add_(0, j_idx, -disp_x)
        dy_acc.index_add_(0, i_idx, disp_y)
        dy_acc.index_add_(0, j_idx, -disp_y)
...
def fast_legalize(cf, margin=1e-4, bin_scale=2.0, iters=None):
    ...
    for _ in range(iters):
        ...
        # Vectorized: convert pairs to tensors
        pairs = list(overlap_pairs)
        i_idx = torch.tensor([p[0] for p in pairs], dtype=torch.long, device=cf.device)
        j_idx = torch.tensor([p[1] for p in pairs], dtype=torch.long, device=cf.device)
        ...
        disp_x = 0.5 * penx * sx * use_x.float()
        disp_y = 0.5 * peny * sy * (~use_x).float()
...
def efficient_zero_overlap_placement(cell_features, pin_features, edge_list, margin=1e-8, util=1.25, wl_iters=6):
    ...
    for _ in range(max(0, wl_iters_local)):
        ...
        cf = fast_legalize(cf, margin=margin, bin_scale=2.0, iters=10 if N >= 1000 else 30)
...
def calculate_overlap_metrics(cell_features):
    """Calculate ground truth overlap statistics using quadtree (O(N log N)), vectorized."""
    N = cell_features.shape[0]
    if N <= 1:
        return {
            "overlap_count": 0,
    ...
    _, overlap_pairs = _quadtree_find_overlaps(positions, widths, heights)
    ...
    dx = np.abs(positions[i_idx, 0] - positions[j_idx, 0])
    dy = np.abs(positions[i_idx, 1] - positions[j_idx, 1])
    min_sep_x = (widths[i_idx] + widths[j_idx]) / 2
    min_sep_y = (heights[i_idx] + heights[j_idx]) / 2
    overlap_x = np.maximum(0, min_sep_x - dx)
    overlap_y = np.maximum(0, min_sep_y - dy)
    overlap_areas = overlap_x * overlap_y

    total_area = sum(areas)
    return {
        "overlap_count": overlap_count,
        "total_overlap_area": float(overlap_areas.sum()),
        "max_overlap_area": float(overlap_areas.max()),
